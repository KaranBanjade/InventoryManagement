...
const operations = require('../utilities/common')(Hospital);
...
router.get('/:id', async (req, res) => {
    try {
        const resp = await operations.get({ id: req.params.id || req.query.id, populate: ['inventories_', 'assets_'] });
...
router.post('/all', async (req, res) => {
    try {
        const resp = await operations.getAll({ ...req.body, populate: ['inventories_', 'assets_'] });
        sendResponse({ res, status: 200, data: resp });
    } catch (err) {
        throwError(res, err);
    }
